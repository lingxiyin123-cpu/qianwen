...
// 从后端API加载统计数据 - 增加重试机制
async function loadStats(attempt = 0, maxAttempts = 3) {
    try {
        ...
        console.error(`从后端加载统计数据失败 (尝试 ${attempt + 1}/${maxAttempts}):`, error);
        
        // 如果还有重试次数，延迟后重试
        if (attempt < maxAttempts - 1) {
            const delay = Math.pow(2, attempt) * 1000; // 指数退避
            console.log(`将在 ${delay}ms 后重试...`);
            setTimeout(() => loadStats(attempt + 1, maxAttempts), delay);
            return;
        }
        
        // 如果所有重试都失败，使用默认值
        console.log('所有重试都失败，使用默认统计值');
        stats = {
            totalUses: 0,
            totalUsers: 0
        };
...
// 增加使用次数（调用后端API） - 增加重试机制
async function incrementUsage(attempt = 0, maxAttempts = 3) {
    try {
        ...
        console.error(`更新统计数据到后端失败 (尝试 ${attempt + 1}/${maxAttempts}):`, error);
        
        // 如果还有重试次数，延迟后重试
        if (attempt < maxAttempts - 1) {
            const delay = Math.pow(2, attempt) * 1000; // 指数退避
            console.log(`将在 ${delay}ms 后重试...`);
            setTimeout(() => incrementUsage(attempt + 1, maxAttempts), delay);
            return;
        }
        
        // 如果所有重试都失败，记录错误并继续执行（不影响摇签功能）
        console.error('所有重试都失败，统计数据更新失败');
    }
}
